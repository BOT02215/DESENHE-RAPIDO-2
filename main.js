...
// criar uma variável para sortear o índice do item a ser desenhado (lembre-se que você pode usar o quickDrawDataset.length para descobrir quantos itens tem nesta array)
randomNo = Math.floor((Math.random() * quickDrawDataset.length) + 1)
console.log()
// criar uma variável para armazenar este item (array[índice])
elementOfArray = quickDrawDataset[randomNo]
// mostrar esta informação na tela. Colocar um texto antes ajuda o usuário a entender como o site funciona. Por exemplo: "esboço a ser desenhado:"+variável
document.getElementById("esboço_a_ser_desenhado").innerHTML = "Esboço a ser Desnhado: " + elementOfArray
...
// Fazer a função updateCanvas. Ela vai ser chamada sempre que o tempo para desenhar acabar ou quando o DoodleNet conseguir reconhecer o desenho corretamente

function updateCanvas() {
    background("white")
    randomNo = Math.floor((Math.random() * quickDrawDataset.length) + 1)
    elementOfArray = quickDrawDataset[randomNo]
    document.getElementById("esboço_a_ser_desenhado").innerHTML = "Esboço a ser Desenhado: " + elementOfArray
}

function preload() {
    // carregar o modelo do DoodleNet (como fizemos em aula)
}
...
function setup() {
    ...
    //-----------------------------------------------
    // *Apenas na Parte 3 do Projeto*:
    // chamar a função de classificar a imagem sempre que o jogador desenhar algo
    //-----------------------------------------------
}
...
function draw() {
    ...
    // Chama a função que checa o tempo e confere se o desenho está correto
    checkSketch()

    if (drawnSketch == elementOfArray) {
        answerHolder = "set"
        score++;
...
//-----------------------------------------------
// Fazer a função de classificar a imagem do canvas
// Fazer a função com os resultados e mostrá-los na tela além de guardar o nome do "chute" em uma variável
//-----------------------------------------------
